...
$value = Box::of(5)
    ...
    ->assert(fn($value) => $value % 2 === 0) // throws a LogicException if the callback returns false
    ->dump() // prints the value
    ->pull(fn($value) => $value * 2); // doubles the value and unboxes it
...
$value = Box::of('Hello World')                               // string
    ...
    ->pipe(fn($value) => $value - 1) // phpstan: Binary operation "-" between array<int, string> and 1 results in an error.
    ->unbox();
```
...
and prevent guaranteed bad code from ever reaching production.
...
in PHP since 7.4, but often spat on by Laravel, which often makes it impossible to infer types statically and therefore
analyze your code properly. This holds especially true for its Pipeline facade, which is absolutely type-unsafe.
